...
static BOOL isEnabled(void) {
    CFPreferencesAppSynchronize(kPrefDomain);
    Boolean exists = NO;
    Boolean val = CFPreferencesGetAppBooleanValue(CFSTR("enabled"), kPrefDomain, &exists);
...
- (void)captureStillImageAsynchronouslyFromConnection:(AVCaptureConnection *)connection
    ...
    if (s == noErr && blockBuffer) {
        ...
        CMVideoFormatDescriptionCreate(
            ...
            (int32_t)fake.size.width,
            (int32_t)fake.size.height,
            NULL, &fmt);
